...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        correct_scores = stable_scores[np.arange(N), y]
        loss = -np.log(correct_scores / np.sum(stable_scores, axis=1))
        loss = np.sum(loss) / N + reg * np.sum(W1 * W1) + reg * np.sum(W2 * W2)
