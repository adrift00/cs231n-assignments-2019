...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    h = np.zeros((N, T, H))
    for i in range(T):
        ht, cache_ = rnn_step_forward(x[:, i, :], ht, Wx, Wh, b)
        h[:, i, :] = ht
        cache.append(cache_)
...
def rnn_backward(dh, cache):
    ...
    dh_t = np.zeros_like(prev_h)
    for i in range(T - 1, -1, -1):
        dh_t += dh[:, i, :]
        dx_t, dh_t, dWx_t, dWh_t, db_t = rnn_step_backward(dh_t, cache[i])
        dx[:, i, :] = dx_t
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = (x, prev_h, prev_c, next_h, next_c,
             Wh, Wx, out, i, f, o, g, tanh_c)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    x, prev_h, prev_c, next_h, next_c, Wh, Wx, out, i, f, o, g, tanh_c = cache
    dtanh_c = dnext_h * o
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    cache = []
    for i in range(T):
        h_t, c_t, cache_ = lstm_step_forward(x[:, i, :], h_t, c_t, Wx, Wh, b)
        h[:, i, :] = h_t
        cache.append(cache_)
...
def lstm_backward(dh, cache):
    ...
    for i in range(T - 1, -1, -1):
        dh_t += dh[:, i, :]
        dx_t, dh_t, dc_t, dWx_t, dWh_t, db_t = lstm_step_backward(dh_t, dc_t, cache[i])
        dx[:, i, :] = dx_t
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
