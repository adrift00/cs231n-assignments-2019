...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3 * 32 * 32, num_classes=10,
        ...
        for i in range(1, self.num_layers + 1):
            ...
            self.params[w_key] = np.random.randn(in_dim, out_dim) * weight_scale
            self.params[b_key] = np.zeros(out_dim)
            if i != self.num_layers and self.normalization is not None: #batchnorm和layernorm都需要
                self.params['gamma%d' % i] = np.ones(out_dim)
                self.params['beta%d' % i] = np.zeros(out_dim)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers + 1):
            ...
            if i == self.num_layers:
                scores, caches[cache_key] = affine_forward(x_out, W, b)
            else:
                x_out, cache_fc = affine_forward(x_out, W, b)
                if self.normalization == 'batchnorm':
                    gamma = self.params['gamma%d' % i]
                    beta = self.params['beta%d' % i]
                    x_out, cache_norm = batchnorm_forward(x_out, gamma, beta, self.bn_params[i - 1])
                elif self.normalization == 'layernorm':

                    gamma = self.params['gamma%d' % i]
                    beta = self.params['beta%d' % i]
                    x_out, cache_norm = layernorm_forward(x_out, gamma, beta, self.bn_params[i - 1])
                else:
                    cache_norm = None
                ...
                if self.use_dropout:
                    ...
                    cache_dropout = None
                caches[cache_key] = (cache_fc, cache_norm, cache_relu, cache_dropout)
        ...
        for i in range(self.num_layers, 0, -1):
            ...
            if i == self.num_layers:
                ...
                cache_fc, cache_norm, cache_relu, cache_dropout = caches[cache_key]
                if self.use_dropout:
