...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        cache = (x, beta, gamma, x_mean, x_num, x_var, x_std, x_hat, eps)
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
...
def batchnorm_backward(dout, cache):
    ...
    x, beta, gamma, x_mean, x_num, x_var, x_std, x_hat, eps = cache
    N, D = x.shape

    # dout=x_hat*gama
    dx_hat = dout * gamma

    # x_hat=inv_x_std*x_num
    inv_x_std = 1 / x_std
    dx_num = dx_hat * inv_x_std
    dinv_x_std = np.sum((dx_hat * x_num), axis=0)

    # inv_x_std=1/x_std
    dx_std = dinv_x_std * (-1) * (x_std ** -2)

    # x_std=np.sqrt(x_var+eps)
    dx_var = dx_std * 0.5 * (x_var + eps) ** (-0.5)

    # x_var=1/N*np.sum(x_num,axis=1)
    dx_num += dx_var * (2 * x_num) / N

    # x_num=x-x_mean
    dx_mean = np.sum(dx_num, axis=0) * (-1)
    dx = dx_num * (1)
...
def layernorm_backward(dout, cache):
    ...
    inv_x_std = 1 / x_std
    dx_num = dx_hat * inv_x_std
    dinv_x_std = np.sum((dx_hat * x_num), axis=1, keepdims=True)
    ...
    # x_var=1/D*np.sum(x_num,axis=1)
    dx_num += dx_var * (2 * x_num) / D

    # x_num=x-x_mean
    dx_mean = np.sum(dx_num, axis=1, keepdims=True) * (-1)
    dx = dx_num * (1)

    # x_mean=1/D*np.sum(x,axis=0)
    dx += dx_mean * (1 / D * np.ones_like(x))
        ...
        for j in range(F):
            for hh in range(Hout):
                for ww in range(Wout):
                    x_for_conv = x_pad[i, :, (stride * hh):(stride * hh + HH), (stride * ww):(stride * ww + WW)]
                    out[i, j, hh, ww] = x_for_conv.ravel().dot(w[j].ravel()) + b[j]
        ...
        for j in range(F):
            for hh in range(H_out):
                for ww in range(W_out):
                    dx_pad[i, :, (stride * hh):(stride * hh + HH), (stride * ww):(stride * ww + WW)] += \
                        w[j, ...] \
                        * dout[i, j, hh, ww]
                    dw[j, ...] += \
...
def max_pool_backward_naive(dout, cache):
    ...
    for h in range(H_out):
        for w in range(W_out):
            max = np.max(x[:, :, (h * stride):(h * stride + pool_height), (w * stride):(w * stride + pool_width)],
                         axis=(2, 3), keepdims=True)  # 一定要keep_dims，否则无法广播，只会返回false
            max_idx = (max == x[:, :, (h * stride):(h * stride + pool_height), (w * stride):(w * stride + pool_width)])
            dx[:, :, (h * stride):(h * stride + pool_height), (w * stride):(w * stride + pool_width)] = \
                max_idx * dout[:, :, h, w][:, :, np.newaxis, np.newaxis]
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    x_mean = np.mean(xr, axis=(2, 3, 4), keepdims=True)
    x_num = xr - x_mean
    x_var = np.var(xr, axis=(2, 3, 4), keepdims=True)
    x_std = np.sqrt(x_var + eps)
    x_hat = x_num / x_std
    x_hat = x_hat.reshape(N, C, H, W)
    out = gamma * x_hat + beta  # gamma是(1,C,1,1)

    cache = (x, beta, gamma, x_mean, x_num, x_var, x_std, x_hat, eps, G)
...
def spatial_groupnorm_backward(dout, cache):
    ...
    x, beta, gamma, x_mean, x_num, x_var, x_std, x_hat, eps, G = cache
    N, C, H, W = x.shape
    # dout=x_hat*gama
    dx_hat = dout * gamma
    ...
    inv_x_std = 1 / x_std
    dx_num = dx_hat * inv_x_std
    dinv_x_std = np.sum((dx_hat * x_num), axis=(2, 3, 4), keepdims=True)
    ...
    # x_var=1/(C//G*H*W)*np.sum(x_num,axis=1)
    dx_num += dx_var * (2 * x_num) / (C // G * H * W)

    # x_num=x-x_mean
    dx_mean = np.sum(dx_num, axis=(2, 3, 4), keepdims=True) * (-1)
    dx = dx_num * (1)

    # x_mean=1/(C//G*H*W)*np.sum(x,axis=0)
    dx += dx_mean * (1 / (C // G * H * W) * np.ones_like(dx_hat))

    dx = dx.reshape((N, C, H, W))
    dgamma = np.sum(dout * x_hat, axis=(0, 2, 3), keepdims=True)  # 需要keep_dims，否则dgamma不是(1,C,1,1)
